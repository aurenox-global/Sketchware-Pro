...
public class Ix {
    ...
    private boolean hasLocalAiNativeLibrary() {
        if (containsLocalAiNativeLibrary(new File(fpu.getPathNativelibs(c.sc_id)))) {
            return true;
        }
        for (String nativeLibraryDirectory : new ManageLocalLibrary(c.sc_id).getNativeLibs()) {
            if (containsLocalAiNativeLibrary(new File(nativeLibraryDirectory))) {
                return true;
            }
        }
        return false;
    ...
    private boolean containsLocalAiNativeLibrary(File file) {
        if (!file.exists()) {
            return false;
        }
        if (file.isFile()) {
            String name = file.getName().toLowerCase();
            return name.endsWith(".so") && (name.contains("llama") || name.contains("ggml") || name.contains("local_ai"));
        }
        ...
        for (File child : children) {
            if (containsLocalAiNativeLibrary(child)) {
                return true;
            }
        }
        return false;
    ...
    public String a() {
        ...
        boolean addRequestLegacyExternalStorage = targetSdkVersion >= 29 && targetSdkVersion <= 30;
        ...
        // when the build runs immediately after the file is recreated/cleaned.
        ArrayList<String> userPermissions = frc.getPermissionList();
        if (userPermissions != null) {
            for (String s : userPermissions) {
                if (s != null && !s.trim().isEmpty()) {
                    writePermission(a, s.trim());
                }
            }
        }
        if (c.isAdMobEnabled) {
        ...
        if (targetSdkVersion < 28 && !buildSettings.getValue(BuildSettings.SETTING_NO_HTTP_LEGACY, BuildSettings.SETTING_GENERIC_VALUE_FALSE)
