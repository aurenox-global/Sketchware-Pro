...
public class yq {
    ...
    public void a(iC projectLibraryManager, hC projectFileManager, eC projectDataManager, ExportType exportingType) {
        ...
        for (ProjectFileBean customView : projectFileManager.c()) {
            for (ViewBean viewBean : eC.a(projectDataManager.d(customView.getXmlName()))) {
                ...
                switch (className) {
                    case "WebView" -> {
                        N.addPermission(jq.PERMISSION_READ_EXTERNAL_STORAGE);
                        N.addPermission(jq.PERMISSION_WRITE_EXTERNAL_STORAGE);
                    }
    ...
    public ArrayList<SrcCodeBean> a(hC projectFileManager, eC projectDataManager, BuiltInLibraryManager builtInLibraryManager) {
        ...
        for (ProjectFileBean activity : projectFileManager.b()) {
            migrateLegacyWebActivityJavaIfNeeded(javaDir, activity, projectDataManager);
    ...
    private void migrateLegacyWebActivityJavaIfNeeded(String javaDir, ProjectFileBean activity, eC projectDataManager) {
        String activityJavaPath = resolveActivityJavaPath(javaDir, activity);
        if (!FileUtil.isExistFile(activityJavaPath)) {
            return;
        }
        ...
        for (ViewBean viewBean : eC.a(projectDataManager.d(activity.getXmlName()))) {
            if (viewBean.type == ViewBean.VIEW_TYPE_WIDGET_WEBVIEW) {
                hasWebViewInLayout = true;
                break;
            }
        }
        // Some old projects have stale Java source that still handles WebView downloads,
        // but inconsistent layout metadata. If the known crashing pattern is present,
        // migrate anyway.
        if (!hasWebViewInLayout && !hasLegacyInlineRequest) {
            return;
        }
        ...
        // Try a surgical in-place patch first: it only rewrites the buggy
        ...
        // from the current Jx generator is applied automatically.
        String regeneratedCode = new Jx(N, activity, projectDataManager).generateCode(isAndroidStudioExport, sc_id);
        FileUtil.writeFile(activityJavaPath, regeneratedCode);
    }

    /**
     * Surgically patches a legacy generated activity that crashes on non-http(s)
     * WebView downloads (e.g. {@code blob:} / {@code data:} URLs produced by
     * import/export buttons in {@code index.html}).
     * <p>
     * Replaces only the buggy {@code <webView>.setDownloadListener(new DownloadListener() {...})}
     * invocation with a safe version delegating to {@code _downloadWebFile(...)}, injects the
     * helper methods + {@code _BlobDownloadBridge} inner class, ensures the required imports
     * and adds the JavaScript bridge. Every other line of the existing source is left intact.
     *
     * @return {@code true} if the file was patched and written, {@code false} otherwise
     */
    private boolean inPlacePatchLegacyWebViewDownload(String activityJavaPath, String existingCode) {
        ...
        String chromePatchedCode = currentCode;
        if (currentCode.contains("_openWebFileChooser(")) {
            chromePatchedCode = Jx.ensureWebChromeClientsOpenFileChooser(currentCode, EOL);
        }
        ...
        // Inject helper methods + _BlobDownloadBridge inner class if missing.
        if (downloadListenerPatched && !currentCode.contains("private void _downloadWebFile(")) {
            ...
            Jx.appendWebViewDownloadHelpers(helpersBuilder, EOL);
            String helpers = helpersBuilder.toString();
            currentCode = currentCode.substring(0, classCloseIndex) + helpers + currentCode.substring(classCloseIndex);
        }
    ...
    /**
     * Locates the closing {@code }} of the top-level class declaration in the file. We skip
     * braces inside strings, characters and comments so nested classes / lambdas / string
     * literals containing braces don't fool the heuristic.
     */
    private int findFinalClassClosingBrace(String source) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if (c == '{') {
                depth++;
                if (!classOpened) classOpened = true;
            } else if (c == '}') {
                depth--;
                if (classOpened && depth == 0) {
                    classCloseIndex = i;
                }
            }
        }
        return classCloseIndex;
    }

    /**
     * Inserts any missing {@code import} declarations required by the WebView download helpers
     * right after the existing import block (or after the {@code package} statement if no
     * imports exist yet).
     */
    private String ensureWebViewDownloadImports(String source) {
        StringBuilder missing = new StringBuilder();
        for (String imp : Jx.WEBVIEW_DOWNLOAD_HELPER_REQUIRED_IMPORTS) {
            String needle = "import " + imp + ";";
            if (!source.contains(needle)) {
                missing.append(needle).append('\n');
            }
        }
        if (missing.length() == 0) {
            return source;
        ...
        // Handle file starting directly with "import "
        if (lastImportEnd < 0 && source.startsWith("import ")) {
            int semi = source.indexOf(';');
            if (semi >= 0) {
                lastImportEnd = semi + 1;
                int next;
                while ((next = source.indexOf("\nimport ", lastImportEnd)) >= 0) {
                    int s = source.indexOf(';', next);
                    if (s < 0) break;
                    lastImportEnd = s + 1;
                }
            }
        ...
        if (lastImportEnd < 0) {
            ...
            if (packageEnd < 0) return source;
            lastImportEnd = packageEnd + 1;
        }

        return source.substring(0, lastImportEnd) + "\n" + missing + source.substring(lastImportEnd);
    }

    private String resolveActivityJavaPath(String javaDir, ProjectFileBean activity) {
        String defaultPath = javaDir + activity.getJavaName();
        if (FileUtil.isExistFile(defaultPath)) {
            return defaultPath;
        }
        ...
        String classMarker = "class " + activity.getActivityName();
        String fallbackPath = "";
        ...
        while (!stack.isEmpty()) {
            ...
            for (File child : children) {
                ...
                if (!child.isFile() || !activity.getJavaName().equals(child.getName())) {
                    continue;
                }
                ...
                String code = FileUtil.readFile(childPath);
                if (code.contains(classMarker)) {
                    return childPath;
                }
            }
        }
    ...
    private boolean needsWebViewSourceMigration(String javaCode) {
        ...
        boolean hasGeneratedWebDownloadListener = javaCode.contains("setDownloadListener")
                && javaCode.contains("onDownloadStart(");

        boolean hasFileChooserSupport = javaCode.contains("onShowFileChooser(")
                || javaCode.contains("_openWebFileChooser(")
                || javaCode.contains("_WEBVIEW_FILE_CHOOSER_REQUEST_CODE");
        ...
        boolean hasGeneratedInitializeLogic = javaCode.contains("void initializeLogic()")
            || javaCode.contains("void _initializeLogic()");
        boolean hasGeneratedLayoutInflation = javaCode.contains("setContentView(R.layout.")
            || javaCode.contains("inflate(R.layout.");
        boolean looksLikeGeneratedActivity = hasGeneratedInitialize
            && hasGeneratedInitializeLogic
            && hasGeneratedLayoutInflation;
        boolean hasWebViewReference = javaCode.contains("WebView");

        // Legacy generated WebView code crashed on non-http(s) downloads by creating
        // DownloadManager.Request directly with Uri.parse(_url). Regenerate immediately
        // when this pattern is found so blob/data URLs are handled safely.
        if (hasLegacyInlineRequest) {
            return true;
        }

        // Very old generated sources may not contain any of the new helper methods yet.
        // If the file still looks generated and has a WebView, regenerate it to apply
        // modern import/export support.
        if (!hasGeneratedWebDownloadListener && !hasDownloadHelper && !hasFileChooserSupport) {
            return looksLikeGeneratedActivity && hasWebViewReference;
        }

        if (!hasGeneratedWebDownloadListener) {
            return looksLikeGeneratedActivity
                    && hasWebViewReference
                    && (hasDownloadHelper || hasFileChooserSupport);
        }

        if (!hasDownloadHelper) {
            return hasLegacyInlineRequest || !hasFileChooserSupport;
        }

        boolean missingBlobSupport = !javaCode.contains("_downloadBlobFromWebView(");
        boolean missingDataSupport = !javaCode.contains("_saveDataUrlToDownloads(");
        boolean missingUnsupportedUrlGuard = !javaCode.contains("Unsupported export URL");
        boolean brokenFileChooserClient = hasFileChooserSupport && Jx.hasWebChromeClientWithoutFileChooser(javaCode);
        return hasLegacyInlineRequest || missingBlobSupport || missingDataSupport
            || missingUnsupportedUrlGuard || brokenFileChooserClient || !hasFileChooserSupport;
    }
