...
public class FileResConfig {
    ...
    public FileResConfig(String sc_id) {
        ...
        try {
            ArrayList<String> parsed = new Gson().fromJson(permissions, Helper.TYPE_STRING);
            if (parsed != null) {
                listFilePermission = parsed;
            }
        } catch (Exception ignored) {
            // Keep default empty list if the saved JSON is malformed so the build never
            // silently drops user-added permissions because of a parser failure.
        }
    }
