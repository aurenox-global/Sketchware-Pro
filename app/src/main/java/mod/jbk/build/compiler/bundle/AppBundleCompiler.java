...
public class AppBundleCompiler {
    ...
    public void createModuleMainArchive() throws IOException {
        ...
        try (var moduleMainZip = new ZipOutputStream(moduleMain)) {
            ...
            var localLibraryManager = new ManageLocalLibrary(builder.yq.sc_id);
            addNativeLibrariesToModule(moduleMainZip, new File(new FilePathUtil().getPathNativelibs(builder.yq.sc_id)));
            for (String nativeLibraryDirectory : localLibraryManager.getNativeLibs()) {
                addNativeLibrariesToModule(moduleMainZip, new File(nativeLibraryDirectory));
            }

            /* Start with enabled Local libraries' JARs */
            List<File> jars = localLibraryManager.getLocalLibraryJars();
    ...
    private void addNativeLibrariesToModule(ZipOutputStream moduleMainZip, File nativeLibrariesDirectory) throws IOException {
        ...
        for (var architecture : architectures) {
            ...
            for (var nativeLibrary : nativeLibraries) {
                ...
                String entryName = MODULE_LIB + File.separator + architecture.getName() + File.separator + nativeLibrary.getName();
                if (addedEntries.contains(entryName)) {
                    continue;
                }

                var nativeZipEntry = new ZipEntry(entryName);
                moduleMainZip.putNextEntry(nativeZipEntry);
                try (var nativeInputStream = new FileInputStream(nativeLibrary)) {
                    var buffer = new byte[1024];
                    int length;
                    while ((length = nativeInputStream.read(buffer)) > 0) {
                        moduleMainZip.write(buffer, 0, length);
                    }
                }
                moduleMainZip.closeEntry();
