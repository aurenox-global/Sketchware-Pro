...
class KotlinCompiler(
    ...
    fun compile() {
        ...
        val extensions = KotlinCompilationPipeline.snapshot()
        ...
        extensions.forEach { extension ->
            runCatching {
                extension.configureArguments(arguments, args, builder, filesToCompile)
            }.onFailure { throwable ->
                LogUtil.w(TAG, "Kotlin extension failed during argument setup: ${extension.javaClass.name}", throwable)
            }
        }
        ...
        val hasErrors = collector.hasErrors()
        extensions.forEach { extension ->
            runCatching {
                extension.onCompilationFinished(!hasErrors, collector.toString(), builder, filesToCompile)
            }.onFailure { throwable ->
                LogUtil.w(TAG, "Kotlin extension failed during result callback: ${extension.javaClass.name}", throwable)
            }
        }
